...
void CpekmenView::DrawGhost(CDC* pDC, CRect rect) {
	...
	int x = rect.TopLeft().x;   // Gornja leva tacka X
	int y = rect.TopLeft().y;   // Gornja leva tacka Y
	int w = rect.Width();       // Sirina pravougaonika
	int h = rect.Height();      // Visina pravougaonika
	int r = w / 2;              // PolupreCnik gornjeg luka
	int waveHeight = h / 6;     // Visina talasa na dnu
	int waveWidth = w / 6;      // Sirina svakog talasa
	...
	pDC->ArcTo(
		...
		CPoint(x, y + h / 2),                  // Pocetna tacka luka
		CPoint(x + w, y + h / 2));             // Krajnja tacka luka
	...
	for (int i = 0; i < 6; i++) {
		...
		pDC->ArcTo(
			...
			CPoint(currentX, y + h - waveHeight / 2),       // Pocetak luka
			CPoint(currentX - waveWidth, y + h - waveHeight / 2)); // Kraj luka
		currentX -= waveWidth; // Pomeraj X koordinatu za cirinu jednog talasa
	}

	// Crtanje leve ivice prema pocetnoj tacki gornjeg luka
	pDC->LineTo(x, y + h / 2);
