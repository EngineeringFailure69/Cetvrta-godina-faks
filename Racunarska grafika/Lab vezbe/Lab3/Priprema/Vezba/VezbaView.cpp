...
void CVezbaView::makeBlue(CBitmap* bitmap) {
	...
	for (int i = 0; i < b.bmWidthBytes * b.bmHeight; i += 4) {
		...
		BYTE gr = min(255, (bits[i] + bits[i + 1] + bits[i + 2]) / 3 + 64);
		bits[i] = gr;
		bits[i + 1] = bits[i + 2] = 0; // B,G,R - 2 preostala kanala se setuju na 0
	}
