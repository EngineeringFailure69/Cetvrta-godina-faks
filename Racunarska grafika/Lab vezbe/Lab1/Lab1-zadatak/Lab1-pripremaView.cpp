...
#pragma region cetke/olovke
	CPen* sivaOlovka = new CPen(PS_SOLID, 1, RGB(230, 230, 230));
	CPen* crvenaOlovka = new CPen(PS_SOLID, 5, RGB(255, 0, 0));
	CPen* tanjaCrvenaOlovka = new CPen(PS_SOLID, 3, RGB(255, 0, 0));
	...
	CBrush* srafuraCetka = new CBrush(/*HS_BDIAGONAL*/HS_FDIAGONAL, RGB(0, 0, 255));
...
#pragma endregion
	...
	pDC->SelectObject(crvenaOlovka);
	Trougao(pDC, CPoint(kvadrat * 1, kvadrat * 1.5), CPoint(kvadrat * 9.5, kvadrat * 10), CPoint(kvadrat * 1, kvadrat * 18.5));
	pDC->SelectObject(narandzastaCetka);
	Trougao(pDC, CPoint(kvadrat * 1, kvadrat * 1.5), CPoint(kvadrat * 7, kvadrat * 1.5), CPoint(kvadrat * 7, kvadrat * 7.4));
	pDC->SelectObject(zutaCetka);
	pDC->Rectangle(kvadrat * 7, kvadrat * 1.5, kvadrat * 13, kvadrat * 7.5);
	pDC->SelectObject(srafuraCetka);
	Trougao(pDC, CPoint(kvadrat * 13, kvadrat * 1.5), CPoint(kvadrat * 19, kvadrat * 1.5), CPoint(kvadrat * 13, kvadrat * 7.5));
	pDC->SelectObject(crvenaCetka);
	Trougao(pDC, CPoint(kvadrat * 19, kvadrat * 1.5), CPoint(kvadrat * 10.5, kvadrat * 10), CPoint(kvadrat * 19, kvadrat * 18.5));
	pDC->SelectObject(zelenaCetka);
	Trougao(pDC, CPoint(kvadrat * 1, kvadrat * 18.5), CPoint(kvadrat * 7, kvadrat * 12.5), CPoint(kvadrat * 13, kvadrat * 18.5));
	pDC->SelectObject(ljubicastaCetka);
	Cetvorougao(pDC, CPoint(kvadrat * 7, kvadrat * 12.5), CPoint(kvadrat * 13, kvadrat * 12.5), CPoint(kvadrat * 19, kvadrat * 18.5), CPoint(kvadrat * 13, kvadrat * 18.5));
	pDC->SelectObject(tanjaCrvenaOlovka);
	pDC->SelectObject(narandzastaCetka);
	DrawRegularPolygon(pDC, kvadrat * 5.2, kvadrat * 3.3, 25, 8, 0);
	pDC->SelectObject(srafuraCetka);
	DrawRegularPolygon(pDC, kvadrat * 14.7, kvadrat * 3.3, 25, 7, 0);
	pDC->SelectObject(rozaCetka);
	DrawRegularPolygon(pDC, kvadrat * 4.5, kvadrat * 10, 40, 5, 0); 
	pDC->SelectObject(zelenaCetka);
	DrawRegularPolygon(pDC, kvadrat * 7, kvadrat * 16, 35, 6, 0);
