...
void CbilijarView::DrawStick(CDC* pDC, int w)
{
	...
	CPen belaOlovka(PS_SOLID, 2, RGB(255, 255, 255));
	...
	pDC->LineTo(startX, startY + wBraon);
	...
	pDC->MoveTo(startX + presekBoja, startY + 2);
	pDC->SelectObject(belaOlovka);
	pDC->LineTo(startX + presekBoja + w - (startX + presekBoja), startY + wOker + 2);
...
void CbilijarView::DrawBall(CDC* pDC, int w)
{
	...
	pDC->Ellipse(5 - w / 2 / 3, -5 - w / 2 / 3, 5 + w / 2 / 3, -5 + w / 2 / 3);
...
void CbilijarView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	else if (nChar == VK_DOWN && !kolizija)
	{
		stapX -= 10;
		kolizija = false;
	}
